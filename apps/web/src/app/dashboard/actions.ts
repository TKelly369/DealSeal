...
export async function getDashboardMetrics(): Promise<DashboardMetrics> {
  ...
  let activeLenderLinks = 0;
  let blockedChecks = 0;
  let totalChecks = 0;

  try {
    [totalDeals, recentDocuments, activeLenderLinks, blockedChecks, totalChecks] = await Promise.all([
      prisma.deal.count({
        where: {
    ...
    console.error("[DealSeal] Dashboard metrics fallback", error);
  }
