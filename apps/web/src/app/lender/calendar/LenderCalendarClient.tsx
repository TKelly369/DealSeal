...
import { createLenderCalendarEventAction } from "./actions";
...
export function LenderCalendarClient({
  events,
  deleteAction,
}: {
  ...
  const [state, formAction] = useActionState(
    async (_prev: { error?: string } | null, formData: FormData) => {
      try {
        await createLenderCalendarEventAction(formData);
        return null;
      } catch (e) {
    ...
    <div style={{ display: "grid", gap: "1.25rem", gridTemplateColumns: "minmax(0, 1fr) minmax(280px, 360px)" }}>
      <div className="card">
        ...
        <p style={{ color: "var(--muted)", fontSize: "0.9rem", marginTop: 0 }}>
          Follow-ups, dealer conditions, document deadlines, repo/replevin review dates, internal notes, and alert
          reminders. Events are listed chronologically.
        </p>
        {byDay.length === 0 ? (
              ...
              >
                ...
                {ev.dealId ? (
                  <p style={{ margin: "0.35rem 0 0", fontSize: "0.82rem" }}>
                    Deal:{" "}
                    <Link
                      href={`/lender/deal-intake/${ev.dealId}`}
                      className="btn btn-secondary"
      ...
      <div className="card" style={{ alignSelf: "start" }}>
        ...
        <form action={formAction} className="ds-form-grid">
          ...
          <label>
            Title
            <input name="title" type="text" required placeholder="e.g. Funding package due" />
          </label>
          <label>
            Starts
          ...
          <label>
            Deal ID (optional)
            <input name="dealId" type="text" placeholder="Must be a deal in your book" />
          </label>
          <label>
            Notes
            <textarea name="notes" rows={3} placeholder="Stips, title clerk, etc." />
          </label>
          <SubmitCalendar />
        </form>
      </div>
